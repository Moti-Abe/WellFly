...
class _MultiCitySearchPageState extends State<MultiCitySearchPage> {
  ...
  void _startSearch() {
    ...
    List<OriginDestination> odList = [
      OriginDestination(
        departure: Departure(
          airportCode: _extractCode(from1Controller.text),
          date: DateFormat('yyyy-MM-dd').format(depart1Date!),
        ),
        arrival: Arrival(airportCode: _extractCode(to1Controller.text)),
      ),
      OriginDestination(
        departure: Departure(
          airportCode: _extractCode(from2Controller.text),
          date: DateFormat('yyyy-MM-dd').format(depart2Date!),
        ),
        arrival: Arrival(airportCode: _extractCode(to2Controller.text)),
      ),
    ];
    ...
    if (addThirdLeg && depart3Date != null) {
      odList.add(
        OriginDestination(
          departure: Departure(
            airportCode: _extractCode(from3Controller.text),
            date: DateFormat('yyyy-MM-dd').format(depart3Date!),
          ),
          arrival: Arrival(airportCode: _extractCode(to3Controller.text)),
        ),
      );
    }

    final request = FlightShoppingRequest(
      originDestinations: odList,
